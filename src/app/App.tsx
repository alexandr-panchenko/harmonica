...
export function App() {
  ...
  const onVirtualStart=(action:HarmonicaAction)=>{signalRef.current={midi:action.canonicalMidi,sounding:true,articulation:signalRef.current.articulation+1};setPlaybackStatus(player.diagnostics);void player.noteOn(action.id,action.canonicalMidi).finally(()=>setPlaybackStatus(player.diagnostics))};
  ...
  const listen = () => { microphone.current.resetTracker(); segmenter.current.reset();setPlaybackStatus(player.diagnostics);const playback=mode === "find" && target?.midi !== undefined?player.playNote(target.midi):player.playMelody(exerciseMelody);void playback.finally(()=>setPlaybackStatus(player.diagnostics)); };
  ...
  const loadSong = (key: string) => { const item=SONGS[key]!; setSongKey(key); setCustomAbc(item.abc); setMelody(parseAbc(item.abc,key)); resetMode("song"); setScreen("game"); };
  ...
  const songDock=mode==="song"?<ControlDock label="Song practice controls">
    ...
    <ControlGroup label="Layout"><div className="segmented" role="group" aria-label="Staff display"><button className={staffMode==="timeline"?"active":""} aria-pressed={staffMode==="timeline"} onClick={()=>setStaffMode("timeline")}>Timeline</button><button className={staffMode==="score"?"active":""} aria-pressed={staffMode==="score"} onClick={()=>setStaffMode("score")}>Score</button></div></ControlGroup>
    ...
    <ControlGroup label="Reference"><button className="listen" onClick={listen}>▶ Listen</button></ControlGroup>
  ...
  return <div className={`app-shell game-shell mode-${mode}`}><header className="game-header"><button className="menu-exit" onClick={exitMode}><span>←</span> Menu</button><div className="game-brand"><span className="brand-mark">H</span><span><b>Harmonica</b><small>Trainer</small></span></div><div className="game-actions"><ThemeControl/><div className="session-stats"><span><b>{session.hits}</b><small>Hits</small></span><span><b>{session.attempts ? Math.round(session.hits/session.attempts*100):100}%</b><small>Accuracy</small></span></div><button className="icon-button" aria-label="Settings" onClick={()=>setShowSettings(!showSettings)}>⚙</button></div></header>
    <main className="play-scene"><div className="mode-strip"><div><span className="eyebrow">{copy[0]}</span><h1>{copy[1]}</h1><p>{copy[2]}</p></div><span className={`feedback-chip ${status}`}>{status==="hit"?"✓":status==="miss"?"↕":"◇"} {message}</span></div>
      <PracticeWorkspace>{songDock}{playbackStatus.status==="degraded"&&<div className="playback-warning" role="status"><span><b>Sampled harmonica is degraded.</b> {playbackStatus.error}</span><button onClick={retryPlayback}>Retry samples</button></div>}
      ...
      {mode!=="song"&&<div className="staff-mode-switch segmented" role="group" aria-label="Staff display"><button className={staffMode==="timeline"?"active":""} aria-pressed={staffMode==="timeline"} onClick={()=>setStaffMode("timeline")}>Timeline</button><button className={staffMode==="score"?"active":""} aria-pressed={staffMode==="score"} onClick={()=>setStaffMode("score")}>Score</button></div>}
      <section className="music-surface" aria-label="Music and transport">{mode!=="find"&&!(mode==="ear"&&!earRevealed)&&<MusicTransportControls state={transportState} meter={exerciseMelody.meter.numerator} onToggle={toggleTransport} onRestart={restartTransport}/>}<MusicStage document={notationDocument} mode={staffMode} activeBeat={stageBeat} activeWrittenEventId={activeWritten?.id} hiddenWrittenEventIds={hiddenWrittenIds} pitchTrace={trace} feedback={status} accessibleLabel={`${copy[1]} notation`} ribbons={mode==="find"||(mode==="ear"&&!earRevealed)?"none":"duration"} showPlayhead={practice==="realtime"&&mode!=="find"&&(mode!=="ear"||earRevealed)} showNoteNames={staffLabels} namingSystem={namingSystem} nowMs={input==="microphone"||practice==="realtime"?performance.now():performanceNow(trace,renderedPerformance)} transport={mode!=="find"&&!(mode==="ear"&&!earRevealed)?{positionBeat:transportState.positionBeat,status:transportState.status,seekable:true,onSeek:seekTransport}:undefined} eventVisuals={eventVisuals} playedMidi={signalRef.current.midi}/><div className={`practice-feedback ${status}`} aria-live="polite">{message}</div></section>
