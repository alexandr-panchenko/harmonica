...
export function App() {
  ...
  const [staffLabels,setStaffLabels]=useState(false), [harmonicaLabels,setHarmonicaLabels]=useState(true), [namingSystem,setNamingSystem]=useState<NamingSystem>("letters");
  const [findRange,setFindRange]=useState<FindRange>("beginner"), [accidentals,setAccidentals]=useState<AccidentalMode>("naturals"), [profileId,setProfileId]=useState(STANDARD_C12.id);
  ...
  const submitSegment = (segment: InputNoteSegment, action?: HarmonicaAction) => {
    ...
    } else { addTimelinePerformance(segment,"incorrect"); flash("miss", midi === target.midi ? "Right note · adjust intonation" : `${teachingNoteName(midi,namingSystem)} · ${midi < (target.midi ?? 0) ? "play higher" : "play lower"}`); window.setTimeout(()=>setPerformance((value)=>value.filter((item)=>item.id!==segment.id)),1400); }
  };
  ...
  const settings=<SettingsDrawer strict={strict} setStrict={setStrict} staffLabels={staffLabels} setStaffLabels={setStaffLabels} harmonicaLabels={harmonicaLabels} setHarmonicaLabels={setHarmonicaLabels} namingSystem={namingSystem} setNamingSystem={setNamingSystem} profileId={profile.id} setProfileId={setProfileId} diagnostics={diagnostics} onClose={()=>setShowSettings(false)}/>;
  ...
  return <div className={`app-shell game-shell mode-${mode}`}><header className="game-header"><button className="menu-exit" onClick={exitMode}><span>←</span> Menu</button><div className="game-brand"><span className="brand-mark">H</span><span><b>HARMONICA</b><small>TRAINER</small></span></div><div className="session-stats"><span><b>{session.hits}</b><small>HITS</small></span><span><b>{session.attempts ? Math.round(session.hits/session.attempts*100):100}%</b><small>ACCURACY</small></span></div><button className="icon-button" aria-label="Settings" onClick={()=>setShowSettings(!showSettings)}>⚙</button></header>
    <main className="play-scene"><div className="mode-strip"><div><span className="eyebrow">{copy[0]}</span><h1>{copy[1]}</h1><p>{copy[2]}</p></div><span className={`feedback-chip ${status}`}>{status==="hit"?"✓":status==="miss"?"↕":"◇"} {message}</span></div>
      ...
      <VirtualHarmonica profile={profile} onStart={onVirtualStart} onEnd={onVirtualEnd} showLabels={harmonicaLabels} namingSystem={namingSystem} detectedMidis={detectedMidis} guideMidis={guideMidis}/>
...
function SettingsDrawer({strict,setStrict,staffLabels,setStaffLabels,harmonicaLabels,setHarmonicaLabels,namingSystem,setNamingSystem,profileId,setProfileId,diagnostics,onClose}:{strict:boolean;setStrict:(value:boolean)=>void;staffLabels:boolean;setStaffLabels:(value:boolean)=>void;harmonicaLabels:boolean;setHarmonicaLabels:(value:boolean)=>void;namingSystem:NamingSystem;setNamingSystem:(value:NamingSystem)=>void;profileId:string;setProfileId:(value:string)=>void;diagnostics?:MicrophoneDiagnostics;onClose:()=>void}) {
  return <aside className="settings-drawer"><button className="close" aria-label="Close settings" onClick={onClose}>×</button><span className="eyebrow">PLAYER SETUP</span><h2>Practice settings</h2><label>Instrument<select value={profileId} onChange={(event)=>setProfileId(event.target.value)}>{HARMONICA_PROFILES.map((item)=><option key={item.id} value={item.id}>{item.name}</option>)}</select></label><fieldset><legend>Learning labels</legend><label><input type="checkbox" checked={staffLabels} onChange={(event)=>setStaffLabels(event.target.checked)}/> Show labels on staff</label><label><input type="checkbox" checked={harmonicaLabels} onChange={(event)=>setHarmonicaLabels(event.target.checked)}/> Show labels on harmonica</label></fieldset><label>Note naming<select value={namingSystem} onChange={(event)=>setNamingSystem(event.target.value as NamingSystem)}><option value="letters">Letters · C D E</option><option value="solfege">Solfège · Do Re Mi (Si)</option></select></label><label><input type="checkbox" checked={strict} onChange={(event)=>setStrict(event.target.checked)}/> Strict intonation ±20¢</label><p>Staff and instrument labels are independent. Solfège uses Do–Re–Mi–Fa–Sol–La–Si consistently.</p><a href="?lab=calibration">Instrument calibration →</a><a href="?lab=fixtures">Fixture recorder →</a><a href="?lab=timing">Timing calibration →</a><pre>{JSON.stringify({...diagnostics,...player.diagnostics},null,2)}</pre></aside>;
}
