...
export function App() {
  ...
  return <div className="app-shell"><header className="topbar"><a className="brand" href="./"><span className="brand-mark">H</span><span><b>Harmonica</b><small>TRAINER</small></span></a><nav>{(["find","score","ear"] as const).map((item)=><button key={item} className={mode===item?"active":""} onClick={()=>resetMode(item)}>{item==="find"?"Find a note":item==="score"?"Play the score":"Play by ear"}</button>)}</nav><div className="top-actions"><a href="?lab=pitch">Labs</a><button aria-label="Settings" onClick={()=>setShowSettings(!showSettings)}>⚙</button></div></header>
    <main><div className="mode-strip"><div><span className="eyebrow">{mode === "find" ? "MODE A · NOTE IDENTITY" : mode === "score" ? "MODE B · SCORE" : "MODE C · EAR"}</span><h1>{mode === "find" ? "Find the note" : mode === "score" ? "Play the score" : "Play it by ear"}</h1></div><div className="session-stats"><span><b>{session.hits}</b> hits</span><span><b>{session.attempts ? Math.round(session.hits/session.attempts*100):100}%</b> identity</span></div></div>
      ...
      {showSettings&&<aside className="settings-drawer"><button className="close" onClick={()=>setShowSettings(false)}>×</button><h2>Practice settings</h2><label><input type="checkbox" checked={strict} onChange={(event)=>setStrict(event.target.checked)}/> Strict intonation ±20¢</label><label><input type="checkbox" checked={showPitchLabels} onChange={(event)=>setShowPitchLabels(event.target.checked)}/> Show pitch labels (learning aid)</label><label>Find-note range<select value={findRange} onChange={(event)=>{setFindRange(event.target.value as FindRange);setActive(0)}}><option value="beginner">Beginner naturals</option><option value="chromatic">One chromatic octave</option><option value="full">Full 12-hole range</option></select></label><p>Pitch labels are off by default in assessment. Identity and intonation are evaluated separately.</p><a href="?lab=calibration">Instrument calibration →</a><a href="?lab=fixtures">Fixture recorder →</a><a href="?lab=timing">Timing calibration →</a><pre>{JSON.stringify({...diagnostics,...player.diagnostics},null,2)}</pre></aside>}
    </main><footer><span>Standard C · 12 holes · A4 440 Hz</span><nav><a href="?lab=pitch">Pitch diagnostics</a><a href="?lab=fixtures">Fixture recorder</a><a href="?lab=timing">Timing</a></nav></footer></div>;
}
