...
import { SongPracticePresetControl } from "../song-practice/SongPractice";
import { SONG_GUIDANCE_PRESETS, type SongGuidancePreset } from "../song-practice/guidancePresets";
...
export function App() {
  ...
  const [songPreset,setSongPreset]=useState<SongGuidancePreset>("practice"),[mistakePolicy,setMistakePolicy]=useState<MistakePolicy>("pause"),[stepEventIndex,setStepEventIndex]=useState(0);
  ...
  const onVirtualStart=(action:HarmonicaAction)=>{signalRef.current={midi:action.canonicalMidi,sounding:true,articulation:signalRef.current.articulation+1};void player.noteOn(action.id,action.canonicalMidi)};
  ...
  const listen = () => { microphone.current.resetTracker(); segmenter.current.reset(); if (mode === "find" && target?.midi !== undefined) void player.playNote(target.midi); else void player.playMelody(exerciseMelody); };
  useEffect(()=>{
    if(mode==="find")return;
  ...
  useEffect(()=>{
    ...
    const tick=()=>{
      ...
      }else if(transport.current.state.status==="playing"){
  ...
  const loadImportedSong = (abc: string) => { try { const imported=parseAbc(abc,"imported");setCustomAbc(abc);setMelody(imported);resetMode(pendingSongMode);setScreen("game") } catch(error) { window.alert(error instanceof Error?error.message:"Invalid ABC") } };
  const resetMode = (next: Mode) => { setMode(next);const preset:SongGuidancePreset=next==="guided"?"learn":"practice";setSongPreset(preset);setPractice(SONG_GUIDANCE_PRESETS[preset].defaultMode); setActive(0);setStepEventIndex(0); setEarPlayed([]); setEarRevealed(false);setEarAssisted(false);setEarSkipped(false); setPerformance([]); setTrace([]); setFlowResults([]); setMessage(next === "ear" ? "Listen first · pitches stay hidden" : next==="find"?"Read the staff, then play":next==="rhythm"?"Choose a pattern, then start Step or In time":"Ready"); };
  const startMode = (next: Mode) => { if(next==="score"||next==="guided"){setPendingSongMode(next);setScreen("songs");return} if(next==="ear")setEarPhrase(generateRandomPhrase(profile,{length:4,maxInterval:5,tempoQpm:92}));if(next==="rhythm")setRhythmMelody(generateRhythm(rhythmSettings));resetMode(next); setScreen("game"); };
  const exitMode = () => { player.stop(); microphone.current.stop(); setInput("virtual"); setScreen("menu"); };
  const stageBeat=mode==="find"?0:mode==="ear"&&!earRevealed?sourceEvents[activeEventIndex]?.startBeat??0:transportState.positionBeat;
  ...
  const activeWritten=notationDocument.writtenEvents[activeEventIndex]??notationDocument.writtenEvents.find(event=>event.kind==="note");
  ...
  const guideAction=((mode==="score"||mode==="guided")&&SONG_GUIDANCE_PRESETS[songPreset].fingering||mode==="rhythm"||(mode==="ear"&&earRevealed))?planned:undefined;
  ...
  const changeSongPreset=(preset:SongGuidancePreset)=>{setSongPreset(preset);setPractice(SONG_GUIDANCE_PRESETS[preset].defaultMode);setMessage(`${SONG_GUIDANCE_PRESETS[preset].label} guidance selected`);};
  ...
  if (screen === "songs") return <SongLibrary mode={pendingSongMode} selected={songKey} initialAbc={customAbc} onChoose={loadSong} onImport={loadImportedSong} onBack={()=>setScreen("menu")}/>;

  const modeCopy:Record<Mode,[string,string,string]>={find:["Staff to instrument","Find the note","Read the target, then find its exact position."],score:["Notation practice","Play the score","Follow the notation one note at a time or perform the full run."],ear:["Listening practice","Play it by ear","Listen to the phrase and discover each interval."],rhythm:["Timing practice","Rhythm training","Match starts, holds, releases and rests on one friendly pitch."],guided:["Visible guidance","Learn a song","Follow the highlighted note and harmonica position. Nothing is hidden."]};
  ...
  return <div className={`app-shell game-shell mode-${mode}`}><header className="game-header"><button className="menu-exit" onClick={exitMode}><span>←</span> Menu</button><div className="game-brand"><span className="brand-mark">H</span><span><b>Harmonica</b><small>Trainer</small></span></div><div className="game-actions"><ThemeControl/><div className="session-stats"><span><b>{session.hits}</b><small>Hits</small></span><span><b>{session.attempts ? Math.round(session.hits/session.attempts*100):100}%</b><small>Accuracy</small></span></div><button className="icon-button" aria-label="Settings" onClick={()=>setShowSettings(!showSettings)}>⚙</button></div></header>
    <main className="play-scene"><div className="mode-strip"><div><span className="eyebrow">{copy[0]}</span><h1>{copy[1]}</h1><p>{copy[2]}</p></div><span className={`feedback-chip ${status}`}>{status==="hit"?"✓":status==="miss"?"↕":"◇"} {message}</span></div>
      {(mode==="score"||mode==="guided")&&<SongPracticePresetControl preset={songPreset} onChange={changeSongPreset}/>}
      {mode==="ear"&&<section className="exercise-lifecycle ear-lifecycle" aria-label="Ear phrase lifecycle"><div className="source-summary"><b>{earPhrase.title} · {earTargets.length} notes · {earRelative?"Relative":"Absolute"}</b><small>{earSource==="song"?`${melody.title} · measures 1–2`:"Playable random phrase · intervals up to 5 semitones"}{earAssisted?" · assisted":""}{earSkipped?" · previous skipped":""}</small></div><label>Source<select aria-label="Ear phrase source" value={earSource} onChange={event=>setEarSource(event.target.value as "random"|"song")}><option value="random">Random phrase</option><option value="song">Song excerpt</option></select></label><div className="segmented" aria-label="Ear mode"><button className={!earRelative?"active":""} onClick={()=>{setEarRelative(false);setEarPlayed([]);setActive(0);setEarRevealed(false)}}>Absolute</button><button className={earRelative?"active":""} onClick={()=>{setEarRelative(true);setEarPlayed([]);setActive(0);setEarRevealed(false)}}>Relative</button></div><button className="listen" onClick={listen}>▶ {earPlayed.length?"Replay":"Listen"}</button><button onClick={newEarPhrase}>New phrase</button><button onClick={skipEarPhrase}>Skip</button><button onClick={()=>setMessage(earPlayed.length?`Next interval goes ${earTargets[earPlayed.length]!-(earTargets[Math.max(0,earPlayed.length-1)]??earTargets[0]!)>=0?"up":"down"}`:"Replay once, then choose an anchor")}>Hint</button><button onClick={()=>{const revealed=earRelative&&earPlayed.length?relativeTargets(earTargets,earPlayed[0]!):earTargets;setEarPlayed(revealed);setEarRevealed(true);setEarAssisted(true);setMessage("Revealed · attempt marked assisted")}}>Reveal</button>{earRevealed&&<button className="primary" onClick={newEarPhrase}>Next phrase</button>}<strong>{earPlayed.length} of {earTargets.length} pitches found</strong></section>}
      {mode==="rhythm"&&<section className="exercise-lifecycle rhythm-lifecycle" aria-label="Rhythm pattern lifecycle"><div className="source-summary"><b>{rhythmSource==="generated"?"Generated":"Preset"} · {rhythmSettings.measures} bars · {rhythmSettings.meter} · {rhythmSettings.difficulty}</b><small>{rhythmSettings.pitchPolicy==="any"?"Any stable pitch":"Fixed C4"} · pattern changes only on New or Next</small></div><label>Source<select aria-label="Rhythm source" value={rhythmSource} onChange={event=>setRhythmSource(event.target.value as "generated"|"preset")}><option value="generated">Generated pattern</option><option value="preset">Preset pattern</option></select></label><label>Meter<select aria-label="Rhythm meter" value={rhythmSettings.meter} onChange={event=>setRhythmSettings(value=>({...value,meter:event.target.value as RhythmMeter}))}><option>4/4</option><option>3/4</option><option>6/8</option></select></label><label>Bars<select aria-label="Rhythm measures" value={rhythmSettings.measures} onChange={event=>setRhythmSettings(value=>({...value,measures:Number(event.target.value)}))}><option value="1">1</option><option value="2">2</option><option value="4">4</option></select></label><label>Difficulty<select aria-label="Rhythm difficulty" value={rhythmSettings.difficulty} onChange={event=>setRhythmSettings(value=>({...value,difficulty:event.target.value as RhythmDifficulty}))}><option value="easy">Easy</option><option value="medium">Medium</option><option value="hard">Hard</option></select></label><label>Pitch<select aria-label="Rhythm pitch policy" value={rhythmSettings.pitchPolicy} onChange={event=>setRhythmSettings(value=>({...value,pitchPolicy:event.target.value as "any"|"fixed"}))}><option value="any">Any stable note</option><option value="fixed">Fixed C4</option></select></label><button onClick={newRhythmPattern}>New pattern</button><button onClick={listen}>▶ Repeat / listen</button></section>}
      <div className="toolbar music-controls">{mode!=="find"&&<div className="segmented" aria-label="Practice style"><button className={practice==="step"?"active":""} onClick={()=>setPractice("step")}>Step</button><button className={practice==="realtime"?"active":""} disabled={mode==="ear"&&!earRevealed} onClick={()=>setPractice("realtime")}>In time</button></div>}{mode==="find"&&<button className="listen" onClick={listen}><span>▶</span> Listen</button>}{mode!=="find"&&!(mode==="ear"&&!earRevealed)&&<label className="mistake-policy">Mistakes<select aria-label="Mistake policy" value={mistakePolicy} onChange={event=>setMistakePolicy(event.target.value as MistakePolicy)} disabled={practice==="realtime"}><option value="pause">Pause and continue</option><option value="restart-note">Restart note</option><option value="restart-measure">Restart measure</option></select></label>}</div>
      ...
      {(mode==="score"||mode==="guided")&&<div className="score-source"><button className="song-change" onClick={()=>{setPendingSongMode(mode);setScreen("songs")}}>♫ {melody.title} <span>Change song →</span></button><label>Tempo <input type="range" min="50" max="160" value={melody.tempoQpm} onChange={(event)=>setMelody((value)=>({...value,tempoQpm:Number(event.target.value)}))}/>{melody.tempoQpm} BPM</label></div>}
      {mode!=="find"&&!(mode==="ear"&&!earRevealed)&&<TransportControls state={transportState} meter={exerciseMelody.meter.numerator} onToggle={toggleTransport} onRestart={restartTransport} onSeek={seekTransport}/>}
      ...
      <MusicStage document={notationDocument} mode={staffMode} activeBeat={stageBeat} activeWrittenEventId={activeWritten?.id} hiddenWrittenEventIds={hiddenWrittenIds} pitchTrace={trace} feedback={status} accessibleLabel={`${copy[1]} notation`} ribbons={mode==="find"||(mode==="ear"&&!earRevealed)?"none":"duration"} showPlayhead={practice==="realtime"&&mode!=="find"&&(mode!=="ear"||earRevealed)} showNoteNames={staffLabels||((mode==="score"||mode==="guided")&&SONG_GUIDANCE_PRESETS[songPreset].noteNames)} namingSystem={namingSystem} nowMs={input==="microphone"||practice==="realtime"?performance.now():performanceNow(trace,renderedPerformance)}/>
...
function MainMenu({onStart,onSettings,settings,profile}:{onStart:(mode:Mode)=>void;onSettings:()=>void;settings:ReactNode;profile:HarmonicaProfile}) {
  const modes: {id:Mode;title:string;description:string;symbol:string}[] = [
    ...
    {id:"score",title:"Play the score",description:"Practise a melody note by note, then play it in time with feedback on pitch, timing and duration.",symbol:"♫"},
    ...
    {id:"guided",title:"Learn a song",description:"Follow visible notation and harmonica guidance to start playing a complete melody immediately.",symbol:"→"},
  ];
...
function SongLibrary({mode,selected,initialAbc,onChoose,onImport,onBack}:{mode:"score"|"guided";selected:string;initialAbc:string;onChoose:(key:string)=>void;onImport:(abc:string)=>void;onBack:()=>void}) {
  const [importAbc,setImportAbc]=useState(initialAbc);
  return <div className="song-library"><header><button className="menu-exit" onClick={onBack}>← Main menu</button><span>Song library</span><ThemeControl/></header><main><div className="library-copy"><span className="eyebrow">{mode==="guided"?"Beginner guidance":"Score practice"}</span><h1>Choose your song</h1><p>{mode==="guided"?"Every note and matching harmonica position will be highlighted as you go.":"Choose a melody, then practise it step by step or in time."}</p></div><div className="song-grid">{Object.entries(SONGS).map(([key,song])=><button key={key} className={`song-card ${key===selected?"selected":""}`} onClick={()=>onChoose(key)}><span className="song-symbol">♫</span><b>{song.title}</b><i>{key===selected?"Current":"Choose"} →</i></button>)}</div><details className="import-source"><summary>Import ABC</summary><p>Use your own monodic ABC notation as a separate source.</p><textarea aria-label="ABC notation" value={importAbc} onChange={(event)=>setImportAbc(event.target.value)}/><button className="primary" onClick={()=>onImport(importAbc)}>Open imported score</button></details></main></div>;
}
...
function TransportControls({state,meter,onToggle,onRestart,onSeek}:{state:PracticeTransportState;meter:number;onToggle:()=>void;onRestart:()=>void;onSeek:(beat:number)=>void}){
  ...
  return <section className="practice-transport" aria-label="Practice transport"><div className="transport-actions"><button className="primary" onClick={onToggle}>{state.status==="playing"||state.status==="count-in"?"Pause":state.status==="paused"?"Resume":state.mode==="realtime"?"Count in + start":"Start Step"}</button><button onClick={onRestart}>Restart</button><span>{state.status==="count-in"?`Count in · ${state.countInBeats} beats`:state.status}</span></div><label className="seek-control">Position<input aria-label="Practice position" type="range" min="0" max={state.totalBeats} step="0.05" value={state.positionBeat} onChange={event=>onSeek(Number(event.target.value))}/></label><div className="transport-position"><b>Measure {measure} · beat {beat.toFixed(1)}</b><span>{formatTime(currentSeconds)} / {formatTime(totalSeconds)}</span></div></section>;
}
...
function formatTime(seconds:number):string{const minutes=Math.floor(seconds/60),remaining=Math.floor(seconds%60);return`${minutes}:${String(remaining).padStart(2,"0")}`}
