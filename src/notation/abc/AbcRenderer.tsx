...
import { beatToPoint, pointToBeat, transportPoints } from "./scoreTransportGeometry";
import type { PracticeEventVisuals } from "./practiceEventState";
...
export interface MusicStageProps {
  ...
  transport?:{positionBeat:number;status:"idle"|"count-in"|"playing"|"paused"|"complete";seekable:boolean;onSeek:(beat:number)=>void};
  eventVisuals?:PracticeEventVisuals; playedMidi?:number;
}
...
export function MusicStage({document,mode,activeBeat,activeWrittenEventId,hiddenWrittenEventIds=new Set(),performedSegments=[],pitchTrace=[],feedback="idle",showNoteNames=false,namingSystem="letters",density="balanced",accessibleLabel,nowMs=performance.now(),ribbons="duration",showPlayhead=false,transport,eventVisuals={},playedMidi}:MusicStageProps){
  ...
  const leadingInset=timeline&&(showPlayhead||transport)?Math.max(0,viewportWidth*.18-FIRST_NOTE_OFFSET):0;
  ...
  useEffect(()=>{const root=renderRoot.current,coordinateRoot=canvasRoot.current;if(!root||!coordinateRoot||!viewportWidth)return;const cleanSource=canvasAbcSource(document.source),renderSource=timeline?timelineAbcSource(cleanSource):cleanSource;const visual=abcjs.renderAbc(root,renderSource,{add_classes:true,staffwidth:staffWidth,scale:ENGRAVING_SCALE,wrap:timeline?undefined:{minSpacing:1.8,maxSpacing:2.8,preferredMeasuresPerLine:viewportWidth<600?2:4},timeBasedLayout:timeline?{minPadding:settings.padding,minWidth:settings.width,align:"left"}:undefined,paddingleft:12,paddingright:12,paddingtop:20,paddingbottom:showNoteNames?58:32});
  ...
  const seekPoints=useMemo(()=>transportPoints(document.writtenEvents,anchors),[document,anchors]),playhead=transport?beatToPoint(seekPoints,transport.positionBeat):undefined;
  ...
  const seekFromPointer=(event:React.PointerEvent<HTMLDivElement>)=>{if(!transport?.seekable||!canvasRoot.current)return;const bounds=canvasRoot.current.getBoundingClientRect(),beat=pointToBeat(seekPoints,event.clientX-bounds.left+canvasRoot.current.scrollLeft,event.clientY-bounds.top+canvasRoot.current.scrollTop,mode);transport.onSeek(Math.max(0,Math.min(document.totalBeats,beat)))};
  return <section className={`music-stage mode-${mode} ${feedback}`} aria-label={accessibleLabel} style={{"--timeline-leading-inset":`${leadingInset}px`} as React.CSSProperties}>
    <div className={`music-viewport${transport?.seekable?" seekable":""}`} ref={viewport} onPointerDown={event=>{manualUntil.current=performance.now()+3000;if(transport?.seekable){event.currentTarget.setPointerCapture(event.pointerId);seekFromPointer(event)}}} onPointerMove={event=>{if(event.currentTarget.hasPointerCapture(event.pointerId))seekFromPointer(event)}} onScroll={()=>{if(!timeline)manualUntil.current=performance.now()+1200}}>
      <div className="music-canvas" ref={canvasRoot}>
        {playhead&&<div className={`notation-playhead ${transport?.status??"idle"}`} aria-label="Playback position" style={{left:playhead.x,top:Math.max(10,playhead.y-62)}}><i/></div>}
        {ribbons==="duration"&&<div className="ribbon-underlay" aria-hidden="true">{geometry.segments.map(segment=>{const event=document.writtenEvents.find(value=>value.id===segment.writtenEventId)!;const sound=document.soundingEvents.find(value=>value.id===segment.soundEventId),visual=eventVisuals[event.id];const progress=visual?.progress??(sound?Math.max(0,Math.min(1,(activeBeat-sound.startBeat)/sound.durationBeats)):event.startBeat<activeBeat?1:0);const eventIndex=document.writtenEvents.findIndex(value=>value.id===segment.writtenEventId),activeIndex=document.writtenEvents.findIndex(value=>value.id===activeWrittenEventId),distance=eventIndex-activeIndex,state=visual?.state??(distance===0?(progress>0?"partial":"active"):distance>0&&distance<=2?"upcoming":"pending");return <span key={segment.writtenEventId} className={`music-ribbon ${state}`} data-written-event-id={segment.writtenEventId} style={{left:segment.startX,top:segment.centerY-3,width:Math.max(5,segment.endX-segment.startX)}}><i style={{width:`${progress*100}%`}}/>{visual?.earlyReleaseAt!==undefined&&<em style={{left:`${visual.earlyReleaseAt*100}%`}}/>}{visual?.state==="correct"&&<b>✓</b>}{visual?.state==="missed"&&<b>○</b>}</span>})}</div>}
        ...
        <div className="music-performance-overlay" aria-hidden="true">
          ...
          {playedMidi!==undefined&&feedback==="miss"&&playhead&&<span className="wrong-pitch-marker" aria-label="Wrong played pitch" style={{left:playhead.x-9,top:yForMidi(playedMidi)-10}}>×</span>}
    ...
    {transport?.seekable&&<label className="accessible-seek">Practice position<input aria-label="Practice position" type="range" min="0" max={document.totalBeats} step="0.05" value={transport.positionBeat} onChange={event=>transport.onSeek(Number(event.target.value))}/></label>}
