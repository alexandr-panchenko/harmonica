...
export function Tuner({ frame, observation, message, expectedOffset = 0 }: { frame: TrackerFrame | null; observation?: MicrophoneObservation; message?: string; expectedOffset?: number }) {
  ...
  const idle = message || (observation?.status === "calibrating" ? `Stay quiet · calibrating ${Math.round(observation.calibrationProgress * 100)}%` : observation?.status === "calibration-contaminated" ? "Tone/noise changed · retrying calibration" : observation?.status === "attack" ? "Tonal attack…" : observation?.status === "release" ? "Display held through release…" : "Listening…");
