...
await shot("song-listen-preview-light-desktop.png","light",async page=>{await openSongMode(page);await seekSong(page,2);await page.getByRole("button",{name:"▶ Listen"}).click()});
await shot("song-listen-preview-light-phone.png","light",async page=>{await openSongMode(page);await seekSong(page,2);await page.getByRole("button",{name:"▶ Listen"}).click()},true);
...
await shot("play-by-ear-listen-preview-light-desktop.png", "light", async page=>{await page.getByRole("button",{name:/Play by ear/}).click();await page.getByRole("button",{name:"▶ Listen"}).click()});
...
await shot("rhythm-listen-preview-light-desktop.png", "light", async page=>{await page.getByRole("button",{name:/Rhythm training/}).click();await page.getByRole("button",{name:"▶ Repeat / listen"}).click()});
